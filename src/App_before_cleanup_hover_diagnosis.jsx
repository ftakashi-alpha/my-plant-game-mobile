...
const difficulties = {
  easy: { name: "やさしい", money: 170, income: 14, risk: 0.7, growth: 1.15 },
  normal: { name: "ふつう", money: 130, income: 12, risk: 1.0, growth: 1.0 },
  hard: { name: "むずかしい", money: 95, income: 10, risk: 1.35, growth: 0.9 },
  research: { name: "研究用調整", money: 130, income: 12, risk: 1.0, growth: 1.0 },
};
...
function createPlots() {
  return Array.from({ length: SIZE * SIZE }, (_, id) => ({
    id,
    infection: id === 14 || id === 21 ? 34 : 0,
    disease: id === 14 || id === 21 ? 12 : 0,
    growth: 8,
    dead: false,
    vector: Math.random() < 0.22,
    pathogen: Math.random() < 0.3,
    effects: [],
  }));
}
...
function weatherInfo(turn) {
  if ([5, 6, 13, 14].includes(turn)) {
    return {
      ...
      diseaseBonus: 0.08,
      vectorBonus: 0,
      text: "水はね・濡れ時間により病原菌が広がりやすい状態です。",
    };
  }
  ...
  if ([11, 12].includes(turn)) {
    return {
      ...
      diseaseBonus: 0,
      vectorBonus: 0.08,
      text: "媒介虫が増えやすく、ウイルス病リスクが高まりやすい状態です。",
    };
  }
  return {
    icon: "⛅",
...
function sumEffects(plot) {
  ...
  return {
    infectionReduction: clamp(active.reduce((s, e) => s + e.infectionReduction, 0), -0.3, 0.9),
    diseaseReduction: clamp(active.reduce((s, e) => s + e.diseaseReduction, 0), -0.3, 0.8),
    vectorReduction: clamp(active.reduce((s, e) => s + e.vectorReduction, 0), 0, 0.9),
    names: active.map((e) => `${e.mark}${e.name} 残${e.remaining >= 999 ? "永続" : e.remaining}`),
  };
}
...
export default function App() {
  ...
  const [research, setResearch] = useState({
    risk: 1.0,
    income: 12,
    initialPathogenPressure: 35,
    initialVectorLevel: 25,
  });
  ...
  const [pathogenPressure, setPathogenPressure] = useState(35);
  const [vectorLevel, setVectorLevel] = useState(25);
  ...
  const [log, setLog] = useState([
    "ゲーム開始。区画を選び、最新設定の防除手段を使ってIPMを実践してください。",
  ]);
  const [lesson, setLesson] = useState(
    "教育モード：化学殺菌剤と化学殺虫剤は別々に管理されます。連用すると、それぞれ殺菌剤耐性・殺虫剤抵抗性が上がります。"
  );
  ...
  const hoveredPlot = hoveredId === null ? null : plots.find((p) => p.id === hoveredId);
  ...
  const hoveredRiskInfo = hoveredPlot
    ? (() => {
        ...
        return {
          eff,
          risk,
          reduction: clamp(baseRisk - risk, 0, 1),
        };
      })()
    : null;
  ...
  function getTargetIds() {
    ...
    if (targetMode === "all") {
      centerIds = plots
        .filter((p) => !p.dead || toolKey === "resistant")
        .map((p) => p.id);
    } else {
      centerIds = selected;
    }
    ...
    if (tool.range === "single") {
      return [...new Set(centerIds)];
    }

    const expanded = centerIds.flatMap((id) => neighbors(id));
  ...
  function getPreviewIds() {
    ...
    if (selected.length > 0) {
      ...
      return [...new Set(selected.flatMap((id) => neighbors(id)))];
    }
  ...
  function applySelectedTool() {
    ...
    if (ids.length === 0) {
      addLog("防除対象が選択されていません。区画をクリックして選択してください。");
      return;
    }

    const cost = calculateCost(ids.length);
    if (money < cost) {
      addLog(`資金不足です。必要資金=${cost}、現在資金=${money}です。`);
      return;
    }

    const t = adjustedTool({ ...tool, id: toolKey });
    const aliveCount = Math.max(1, plots.filter((p) => !p.dead).length);
    const coverage = clamp(ids.length / aliveCount, 0, 1);
    ...
    setPlots((prev) =>
      prev.map((p) => {
        ...
        if (toolKey === "roguing") {
          return { ...p, dead: true, disease: 0, infection: 0, effects: [] };
        }
        ...
        return {
          ...
          dead: toolKey === "resistant" ? false : p.dead,
          infection: toolKey === "resistant" ? Math.max(0, p.infection - 20) : p.infection,
          disease: toolKey === "resistant" ? Math.max(0, p.disease - 25) : p.disease,
          effects: [
            ...p.effects.filter((e) => e.key !== toolKey || e.remaining <= 0),
            {
              key: toolKey,
              name: t.name,
              mark: t.mark,
              ...
              infectionReduction: t.infectionReduction,
              diseaseReduction: t.diseaseReduction,
              vectorReduction: t.vectorReduction,
            },
          ],
        };
      })
    ...
    if (toolKey === "fungicide") {
      setFungicideRes((v) => clamp(v + 0.12, 0, 1));
    }
    if (toolKey === "insecticide") {
      setInsecticideRes((v) => clamp(v + 0.12, 0, 1));
    }
    if (toolKey === "microbe") {
      setFungicideRes((v) => clamp(v - 0.04, 0, 1));
    }
    if (toolKey === "enemy" || toolKey === "net") {
      setInsecticideRes((v) => clamp(v - 0.04, 0, 1));
    }

    setActions((prev) => ({ ...prev, [toolKey]: (prev[toolKey] || 0) + 1 }));

    addLog(`${tool.mark} ${tool.name}を実施しました。中心区画=${selected.length}、実際の影響区画=${ids.length}、費用=${cost}。`);
    educate(`【防除解説】${tool.mark} ${tool.name}\n${tool.edu}`);
  }
  ...
  function useItem(key) {
    ...
    if (key === "microbeBoost") {
      setFungicideRes((v) => clamp(v - 0.25, 0, 1));
    }
    if (key === "enemyBoost") {
      setInsecticideRes((v) => clamp(v - 0.25, 0, 1));
    }
  ...
  function nextTurn() {
    ...
    const vectorIncrease = vectorEvent ? Math.round(10 + Math.random() * 16) : Math.round(Math.random() * 6);
    ...
    const nextPlots = plots.map((p) => {
      ...
      const growth = clamp(p.growth + 4 * difficulty.growth * crop.growthFactor - disease / 40, 0, 100);

      const nextEffects = p.effects
        .map((e) => ({ ...e, remaining: Math.max(0, e.remaining >= 999 ? 999 : e.remaining - 1) }))
  ...
  return (
    <div style={styles.app}>
      <section style={styles.card}>
        <h1 style={styles.h1}>植物病害防除ゲーム：最新設定復元版</h1>
        <p style={styles.text}>
          化学殺菌剤と化学殺虫剤を分離し、拮抗微生物、天敵、防虫ネット、雨除け、土壌熱消毒、光防除、
          免疫誘導剤、肥料、抵抗性品種、抜取、薬剤ローテーションを使える設定に戻しました。
        </p>
      ...
      <div style={styles.twoColumn}>
        <section style={styles.card}>
          ...
          <p style={styles.text}>
            区画をクリックして選択します。範囲型の防除は、選択区画を中心に周囲8区画にも効果が入ります。青い太枠は中心区画、青い破線と「影響」表示は効果が及ぶ区画です。
          </p>
          ...
          <div style={styles.buttonLine}>
            ...
            <button style={styles.button} onClick={applySelectedTool} disabled={gameOver}>
              {tool.mark} {tool.name}を実施
            </button>
            <button style={styles.button} onClick={nextTurn} disabled={gameOver}>1ターン進める</button>
              ...
              return (
                ...
                >
                  ...
                  <div style={{ fontSize: 12 }}>
                    {p.vector && !p.dead ? "🐛" : ""}{p.pathogen && !p.dead ? "🧫" : ""}
                  </div>
                </button>
              );
            })}
        ...
        <section style={styles.card}>
          ...
          <p style={styles.text}>
            選択中：<b>{tool.mark} {tool.name}</b> / 範囲：<b>{tool.range === "single" ? "単一区画のみ" : "中心＋周囲8区画"}</b>
            <br />
            {tool.range === "single"
            ...
            {Object.entries(tools).map(([key, t]) => (
              ...
              >
                ...
                <span style={{ fontSize: 11 }}>
                  感染{pct(t.infectionReduction)} / 病勢{pct(t.diseaseReduction)}
                </span>
              </button>
            ))}
          </div>
      ...
      <div style={styles.twoColumn}>
        <section style={styles.card}>
          ...
          {!hoveredPlot ? (
            <p style={styles.text}>
              区画にカーソルを合わせると、その区画の感染度、病勢、防除効果、推定感染リスクが表示されます。
            </p>
          ) : (
            <div style={styles.detailBox}>
              <b>区画 {hoveredPlot.id + 1}：{getStatus(hoveredPlot).label}</b>
              <div style={styles.smallText}>
                感染度={Math.round(hoveredPlot.infection)} / 病勢={Math.round(hoveredPlot.disease)} / 成長={Math.round(hoveredPlot.growth)}
              </div>
              <div style={styles.smallText}>
                媒介虫={hoveredPlot.vector ? "あり" : "低い"} / 病原菌={hoveredPlot.pathogen ? "あり" : "低い"}
              </div>
              <div style={styles.smallText}>
                防除前推定感染確率={pct(baseRisk)} / 防除後推定感染確率={pct(hoveredRiskInfo.risk)} / 低下量={pct(hoveredRiskInfo.reduction)}
              </div>
              <div style={styles.smallText}>
                感染低下={pct(hoveredRiskInfo.eff.infectionReduction)} / 病勢抑制={pct(hoveredRiskInfo.eff.diseaseReduction)} / 媒介虫低下={pct(hoveredRiskInfo.eff.vectorReduction)}
              </div>
              <div style={styles.smallText}>
                有効防除：{hoveredRiskInfo.eff.names.length === 0 ? "なし" : hoveredRiskInfo.eff.names.join("、")}
              </div>
            </div>
          )}
        </section>
        ...
        <section style={styles.card}>
          ...
          {!hoveredPlot ? (
            <p style={styles.text}>
              区画にカーソルを合わせる、または区画をクリックすると、その区画の詳細が表示されます。
            </p>
          ) : (
            <div style={styles.detailBox}>
              <b>区画 {hoveredPlot.id + 1}：{getStatus(hoveredPlot).label}</b>
              ...
              <div style={styles.smallText}>
                現在選択中の防除：{tool.mark} {tool.name} / 影響範囲：
                {tool.range === "single" ? "この区画のみ" : "この区画を中心とする周囲8区画"}
              </div>
            </div>
          )}
        </section>
              ...
              {selectedPlots.map((p) => {
                ...
                return (
                  <div key={p.id} style={styles.detailBox}>
                    <b>区画 {p.id + 1}：{st.label}</b>
                    <div style={styles.smallText}>
                      感染度={Math.round(p.infection)} / 病勢={Math.round(p.disease)} / 成長={Math.round(p.growth)}
                    </div>
                    <div style={styles.smallText}>

...
export default function App() {
  ...
  const hoveredRiskInfo = hoveredPlot
    ? (() => {
        ...
        const reduction = clamp(baseRisk - risk, 0, 1);
        return { eff, risk, reduction };
      })()
    : null;
  ...
  function getTargetIds() {
    let baseIds = [];
    if (targetMode === "all") {
      baseIds = plots.filter((p) => !p.dead || toolKey === "resistant").map((p) => p.id);
    } else {
      baseIds = selected;
    }
    ...
    const expanded = baseIds.flatMap((id) => neighbors(id));
  ...
  function applySelectedTool() {
    ...
    addLog(`${tool.mark} ${tool.name}を${ids.length}区画に実施しました。費用=${cost}。`);
              ...
              return (
                <button
                  ...
                  style={{
                    ...
                    background: st.color,
                    color: st.text,
                    outline: isSelected ? "4px solid #2563eb" : "1px solid rgba(0,0,0,0.12)",

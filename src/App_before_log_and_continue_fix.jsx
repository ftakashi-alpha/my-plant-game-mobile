...
function weatherInfo(turn) {
  if ([5, 6, 13, 14].includes(turn)) {
    return { icon: "🌧️", name: "長雨", diseaseBonus: 0.08, vectorBonus: 0, text: "水はね・濡れ時間により病原菌が広がりやすい状態です。" };
  }
  if ([9, 18, 19].includes(turn)) {
    return { icon: "🌬️", name: "強風", diseaseBonus: 0.06, vectorBonus: 0.02, text: "風で病原菌や媒介虫が移動しやすい状態です。" };
  }
  if ([11, 12].includes(turn)) {
    return { icon: "☀️", name: "高温乾燥", diseaseBonus: 0, vectorBonus: 0.08, text: "媒介虫が増えやすく、ウイルス病リスクが高まりやすい状態です。" };
  }
...
export default function App() {
  ...
  function applySelectedTool(selectedToolKey = toolKey) {
    if (gameOver) return;

    const currentTool = tools[selectedToolKey];

    if (!currentTool) {
      addLog("防除手段が正しく選択されていません。");
      return;
    }
    ...
    if (targetMode === "all") {
      centerIds = plots
        .filter((p) => !p.dead || selectedToolKey === "resistant")
        .map((p) => p.id);
    } else {
      centerIds = selected;
    }
    ...
    if (centerIds.length === 0) {
      addLog(`防除対象が選択されていません。${currentTool.name}を実行するには、先に区画をクリックしてください。`);
      educate(`【操作案内】${currentTool.mark} ${currentTool.name}を実行するには、先に圃場マップ上の区画をクリックして選択してください。`);
      return;
    }
    ...
    const cost =
      targetMode === "all"
        ? Math.ceil(currentTool.cost * 2.2)
        : currentTool.range === "single"
        ? Math.ceil(currentTool.cost * 0.75 * targetIds.length)
        : Math.ceil(currentTool.cost * 0.35 * targetIds.length);

    if (money < cost) {
      addLog(`資金不足です。${currentTool.name}の必要資金=${cost}、現在資金=${money}です。`);
    ...
    addLog(
      `${currentTool.mark} ${currentTool.name}を実施しました。中心区画=${centerIds.length}、実際の影響区画=${targetIds.length}、費用=${cost}。`
    );

    educate(`【防除解説】${currentTool.mark} ${currentTool.name}\n${currentTool.edu}`);
  }
    ...
    <div style={styles.app}>
      ...
      <div style={styles.twoColumn}>
        <section style={styles.card}>
          ...
          <p style={styles.text}>
            青太枠＝選択中の中心区画、青破線＝現在選択している防除手段の影響予定区画、
            緑枠・継続＝防除効果が継続中の区画です。
          </p>
          ...
          <div style={styles.fieldGrid}>
            {plots.map((p) => {
              ...
              const isActive = activeEffectIds.includes(p.id);
        ...
        <section style={styles.card}>
          ...
          <p style={styles.text}>
            防除手段をクリックすると、選択区画に即実行されます。現在選択中：<b>{tool.mark} {tool.name}</b> / 範囲：
              ...
              {selectedPlots.map((p) => {
                ...
                return (
                  <div key={p.id} style={styles.detailBox}>
                    ...
                    <div style={styles.smallText}>
                      防除継続：{eff.names.length === 0 ? "なし" : eff.names.join("、")}
                    </div>
                  </div>
                );
              })}

...
export default function App() {
  ...
  function applySelectedTool(selectedToolKey = toolKey) {
    ...
    if (centerIds.length === 0) {
      const msg = `防除対象が選択されていません。${currentTool.name}を実行するには、先に区画をクリックしてください。`;
    ...
    const msg = `${currentTool.mark} ${currentTool.name}を実施しました。中心区画=${centerIds.length}、実際の影響区画=${targetIds.length}、費用=${cost}。`;
    setLog((prev) => [msg, ...prev].slice(0, 15));

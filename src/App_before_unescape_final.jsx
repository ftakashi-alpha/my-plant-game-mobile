      ...
      <div style={styles.twoColumn}>
        <section className="mobile-section mobile-section-field" style={styles.card}>
          ...
          {React.createElement(
            "div",
            {
              className: "mode-status-banner-safe",
              style:
                interactionMode === "apply"
                  ? styles.modeStatusApply
                  : styles.modeStatusInspect,
            },
            React.createElement(
              "div",
          ...
          <p style={styles.text}>
            通常は区画クリックで情報を確認します。防除したい場合は、操作モードを「防除実施」にしてから区画をクリックまたはタップしてください。範囲型はクリック/タップ区画を中心に最大9区画へ作用します。単一区画型はその区画のみに作用します。青破線＝影響予定区画、緑枠・継続＝防除効果が継続中の区画です。
          </p>
        ...
        <section className="mobile-section mobile-section-tools" style={styles.card}>
          ...
          <div style={styles.modeSwitchBox}>
            <div style={{ fontSize: 13, color: "#52606d", marginBottom: 6 }}>
              区画クリック時の動作
            </div>
            <div style={{ display: "flex", gap: 8, flexWrap: "wrap" }}>
              <button
                type="button"
                style={interactionMode === "inspect" ? styles.modeButtonActive : styles.modeButton}
              ...
              <button
                type="button"
                style={interactionMode === "apply" ? styles.modeButtonActive : styles.modeButton}
            ...
            <div style={styles.smallText}>
              情報確認：区画詳細を見るだけです。防除実施：区画クリックで選択中の防除を実施します。
            </div>
          </div>
          ...
          <div style={styles.defenseGrid}>
            {Object.entries(tools).map(([key, t]) => (
              <button
                ...
                onClick={() => {
                  setToolKey(key);
                  setInteractionMode("apply");
                  educate(`【防除選択】${t.mark} ${t.name}\n${t.edu}\n\n操作モードを「防除実施」にしました。次に圃場マップ上の区画をクリックすると実施されます。`);
                }}
              >
                <b>{t.mark} {t.name}</b>
                <br />

...
export default function App() {
  ...
  function applySelectedTool(selectedToolKey = toolKey) {
    ...
    if (centerIds.length === 0) {
      addLog("防除対象が選択されていません。先に区画をクリックして選択してください。");
    ...
    const cost =
      targetMode === "all"
        ...
        : currentTool.range === "single"
        ? Math.ceil(currentTool.cost * 0.75 * ids.length)
        : Math.ceil(currentTool.cost * 0.35 * ids.length);
    ...
    addLog(
      `${currentTool.mark} ${currentTool.name}を実施しました。中心区画=${centerIds.length}、実際の影響区画=${ids.length}、費用=${cost}。`
    );
